...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your people-api account.<br>
        ...
        &mdash; people-api Team
      `
      return sendMail({ toEmail: email, subject: 'people-api - Password Reset', content })
    })
